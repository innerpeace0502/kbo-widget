...
class KboWidgetProvider : AppWidgetProvider() {
                ...
                override fun onResponse(call: Call, response: Response) {
                    ...
                    for (i in 0 until scores.length()) {
                        ...
                        if ((sAway == away && sHome == home) || (sAway == home && sHome == away)) {
                            ...
                            // 기본: info행 첫칸(game_time)·별도 채널행 노출. 라이브(1)에서만 숨겨 한 줄 바로 대체.
                            ...
                            when (status) {
                                "1" -> {
                                    // 라이브 — 상황을 한 줄 바(이닝·LIVE/채널·베이스·B·S·O) 비트맵 한 장으로 표시.
                    ...
                    } catch (e: Exception) { println("[스코어] 응답 처리 오류: ${e.message}"); restoreCachedScore(context, appWidgetManager, appWidgetId, away, home, prefs) }
                }
            })
    }
    ...
    private fun fetchChannel(
        ...
        KboCommon.httpClient.newCall(Request.Builder()
